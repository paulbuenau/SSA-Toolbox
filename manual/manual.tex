...
%\section{The SSA Algorithm in a Nutshell}
%
%Since the solution is undetermined up to scaling, sign 
%and linear transformations within the \s- and \n-space, we set the
%estimated \s-sources to zero mean and unit variance
%by centering and whitening of the data, i.e.~we write the estimated
%demixing matrix as $\hat{A}^{-1} = \hat{B}W$ where $W =
%\Cov(x)^{-1/2}$ is a whitening matrix and $\hat{B}$ is an orthogonal
%matrix\footnote{This is also standard approach in ICA \cite{ICABook,tdsep}.}.
%The rotation part $\hat{B}$ of the demixing is determined 
%using an optimization procedure such that the estimated stationary sources, 
%i.e.~the first $d$ components of the estimated sources 
%$\mathbf{\hat{s}}(t) = \hat{B}W \mathbf{x}(t)$, are as stationary as possible. Therefore, 
%we split the data into $N$ consecutive epochs $\cset{X}_1, \ldots, \cset{X}_N \subset \R^D$
%and consider estimated sources as stationary if their joint distribution remains 
%unchanged over all epochs. In order to limit the influence of estimation
%errors due to finite samples, we characterize the distribution in each
%epoch $\cset{X}_i$ by its empirical mean $\hat{\boldsymbol \mu}_i$ and covariance matrix
%$\hat{\Sigma}_i$ only. To generically compare distributions up to their
%first two moments we use the Kullback-Leibler divergence $\KLD$ between
%Gaussians, which is the maximum-entropy distribution that is consistent
%with the specified moments \cite{Jay57}. Note that we do {\em not} assume the data 
%to be Gaussian, we only assume that the non-stationarities are visible in 
%the first two moments. 

%Let $\hat{\boldsymbol \mu}_i$ and $\hat{\Sigma}_i$ be the empirical mean and 
%covariance matrix of epoch $i$. Then, for a given rotation $B^\top B = \I$, 
%the mean and covariance of the estimated \s-sources are given by
%\begin{align*}
%  \hat{\boldsymbol \mu}^\s_i = \I^d B W \hat{\boldsymbol \mu}_i 
%  \; \text{ and } \;
%  \hat{\Sigma}^\s_i = \I^d BW \hat{\Sigma}_i (\I^d B W)^\top
%\end{align*}
%respectively, where $\I^d$ denotes the identity matrix truncated to the first
%$d$ rows. Since we have set estimated \s-sources to zero mean and unit covariance,
%minimizing the KL-divergence across all epochs is equivalent to minimizing 
%the KL-divergence between the distribution in each epoch and the multivariate
%standard normal distribution. 
%
%Starting with a random orthogonal matrix, in each step of the optimization 
%we aim to find an orthogonal update $R^\top R = \I$ of the current rotation $B_0$
%such that $B \gets R B_0$ minimizes the loss function
%\begin{eqnarray}
%\label{eq:lossfun}
%L_{B_0}(R) &=& \sum_{i=1}^N
%            \KLD \left[ \N(\hat{\boldsymbol \mu}^\s_i,\hat{\Sigma}^\s_i) \; || 
%            \; \N(0,I) \right]  \nonumber \\ 
%     &=& \sum_{i=1}^N
%          \left( -\log\det \hat{\Sigma}^\s_i + \hat{\boldsymbol \mu}^{\s\top}_i \hat{\boldsymbol \mu}^\s_i \right) .
%\end{eqnarray}
%
%If changes in the first moment shall not be considered, then the loss
%function simplifies to 
%\begin{equation}
%\label{eq:lossfun_nomeans}
%L_{B_0}(R) 
%     = - \sum_{i=1}^N
%         \log\det \hat{\Sigma}^\s_i .
%\end{equation}

%In order to determine the update rotation $R$, we use the fact
%that the manifold of orthogonal matrices is connected to the set of skew-symmatrices 
%$M = -M^\top$ via the exponential map \cite{Bak02}. That is, every orthogonal matrix $R$ 
%can be expressed as $R = e^M$ for some skew-symmetric matrix $M$.  Thus, the gradient of the loss 
%function $L_{B_0}$ w.r.t.~$M$ at $M = 0$ provides us with a search direction $H=-H^\top$ in the set 
%of skew symmetric  matrices. Then we find the update $R$ by performing a line-search 
%\cite{Nocedal99,Plu05} along the one-parameter subgroup $\{ e^{t H} \; | \; t \in \R \}$ 
%of $SO(D)$ to determine the optimal step length $t^*$ such that $R = e^{t^* H}$. 

\bibliographystyle{plain}
\bibliography{../../bib/stable}
