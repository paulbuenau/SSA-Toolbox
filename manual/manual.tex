...
the eigenvalues $\lambda_1, \ldots, \lambda_{D-d}$. The log eigenvalues are drawn such that they 
are uniformly distributed, either on the interval $[ \log v_\text{min}, \log v_\text{max}]$ (with probability $p$) 
or on the interval $[- \log v_\text{max}, - \log v_\text{min}]$ (with probability $1-p$), where 
$1 < v_\text{min} < v_\text{max}$. 

For each epoch, the canonical 
...
%  \frac{ \prod_{x \in \mathcal{X}} p_{\mathcal{N}}(x ; 0, I)  } 
%  { 
