...
it. If not, a purely Java-based library, COLT, will be used. Data and results can imported and
exported as comma-separated values (CSV) files, the fail-safe format of last resort, and through
Matlab's proprietory mat files, a de-facto standard in the Machine Learning community.
...
The SSA Toolbox comes with a self-contained Matlab example script (\texttt{ssa\_example.m}), which generates
data, applies SSA and visualizes the result. 
...
the result, possibly also using appropriate statistical hypothesis tests.
...
\paragraph{\texttt{testSetCustomEpochDefinition()}}  This test ...

\paragraph{TestData}
This class contains tests for the class \texttt{Data}. It consists of two test cases,
\texttt{testSetNumberOfEqualSizeEpochs()} and \texttt{testSetCustomEpochDefinition()}. Both methods check whether an exception is
thrown if we have less data points than epochs (for equally sized epochs and when using a custom epoch definition, respectively).

\paragraph{TestSSA}
This class consists of tests for the class \texttt{SSA}. The methods \texttt{testOptimizeMeanCov()}, \texttt{testOptimizeMean()} and
\texttt{testOptimizeCovariance()} check whether exceptions are thrown if the
determinacy bounds are violated (when using both means and covariance matrices, means only and covariance matrices only, respectively).
The function \texttt{testOptimize()} contains a simple test which checks
