...
\begin{align*}
	\tilde{\sigma} = \sum_{i=1}^n \sum_{j=1}^{D-d} \left|\log \left[ (\Lambda_i)_j \right]\right| .
\end{align*}
Similarly, the total non-stationarity in the mean $\tilde{\mu}$ is given by, 
\begin{align*}
...
In order to measure the accuracy of SSA when the true solution is known, we define an error
measure between the true and the estimated non-stationary subspace. This \textit{subspace error}
is based on the \textit{principle angles} between two subspace, a measure of the relative position
of two linear subspaces. Let $U$ and $V$ be two linear subspaces of the vector space $\mathcal{V}$ 
with $\dim U \leq \dim V$. The first principle angle $\theta_1$ is defined as the smallest 
angle between any two vectors $u \in U$ and $v \in V$, 
\begin{align*}
...
orthonormal bases for the spaces $U$ and $V$ respectively. 

The aim of SSA is to eliminate non-stationary contributions from the estimated stationary sources, 
i.e.~project orthogonal to the non-stationary subspace. We therefore measure the deviation from the true
non-stationary subspace, spanned by the columns of $A^\n$, and its estimate, spanned by the columns of
$\hat{A}^{\n}$, in terms of the principle angles between these two subspaces. If all principle
angles are zero, SSA has identified the true solution and hence the error is zero. However, for every 
non-zero principle angle $\theta_k > 0$, the estimated \s-projection $\hat{P}^\s = (\hat{A}^{\n})^\perp$ 
adds non-stationary contributions to the estimated stationary sources. This affects the mean and the variance 
of the estimated \s-sources as follows. Let $v_k$ be the principle vector in the true non-stationary space
corresponding to the non-zero principle angle $\theta_k$ and let $\mu^\n_k(t)$ and $\sigma^{\n2}_k(t)$ be the mean 
and the variance along that direction respectively. Then $\sin^2 (\theta_k) (\mu_k(t))^2$ will be added to 
the squared norm of the expectation of the estimated \s-sources, and $\sin^2 (\theta_k) \sigma^2_k(t)$ to the 
sum of the variances at time point $t$. 
